...
export default function PrivacyPage() {
  return (
    <article className="container-shell max-w-4xl pb-24 pt-40">
      ...
      <p className="mt-6 text-sm text-muted">Dernière mise à jour : 18 septembre 2026</p>
      <div className="mt-12 space-y-10 leading-8 text-muted">
        ...
        <Section title="Vos droits">Vous pouvez demander l’accès, la rectification ou la suppression de vos données en utilisant notre <Link className="text-ivory underline decoration-gold underline-offset-4" href="/contact">page de contact</Link>. Nous traiterons votre demande conformément au droit applicable.</Section>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) { return <section><h2 className="font-display text-2xl font-semibold text-ivory">{title}</h2><p className="mt-3">{children}</p></section>; }
