...
export default function PhotoCreditsPage() {
  return (
    <main className="container-shell bg-paper pb-24 pt-40 text-charcoal">
        ...
        {FLEET_PHOTO_SOURCES.map((source) => (
          <li className="grid gap-3 py-7 md:grid-cols-[1fr_1.5fr]" key={source.localPath}>
            <div>
              ...
              <p className="mt-1 text-sm text-stone">{source.generation}</p>
            </div>
            <div className="text-sm leading-7 text-stone">
              <p>Photographie : {source.author}</p>
              <p>{source.note}</p>
              <div className="mt-2 flex flex-wrap gap-x-5">
                <a className="min-h-11 py-2 text-charcoal underline decoration-gold underline-offset-4" href={source.sourceUrl} rel="noreferrer" target="_blank">
                  Voir la source
                </a>
                <a className="min-h-11 py-2 text-charcoal underline decoration-gold underline-offset-4" href={source.licenseUrl} rel="noreferrer" target="_blank">
                  {source.license}
                </a>
              </div>
            </div>
